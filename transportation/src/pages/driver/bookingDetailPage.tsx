...
import { 
  ArrowLeft, MapPin, Calendar, User, Mail, 
  CreditCard, Armchair, Clock, Printer, 
  CheckCircle2, AlertCircle, Bus, Tag, Fingerprint,
  ShieldCheck, Hash
} from "lucide-react";
...
const BookingDetailPage = () => {
  ...
  if (loading) {
    return (
      <div className="h-96 w-full flex flex-col items-center justify-center">
        ...
        <p className="text-slate-500 font-black uppercase tracking-widest text-[10px]">Verifying Ticket...</p>
      </div>
    );
  }
  ...
  if (!order) return (
    <div className="text-center p-20">
        ...
        <button onClick={() => navigate(-1)} className="mt-4 text-emerald-500 text-sm font-bold">Return to List</button>
    </div>
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
            <div className="px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase text-emerald-500 tracking-widest">Verified Booking</span>
            </div>
          ...
          <div className="bg-slate-900 border border-white/5 rounded-[3rem] p-8 md:p-12 relative overflow-hidden">
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-12">
                    <div>
                        <p className="text-[10px] font-black uppercase text-slate-500 tracking-[0.3em] mb-2">Bus Service</p>
                        <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter">
                            {order.busName} <span className="text-emerald-500">[{order.busType}]</span>
                        </h2>
                        <p className="text-xs font-bold text-slate-400 mt-1 opacity-60">Plate: {order.busNumber}</p>
                    </div>
                    <div className={`px-4 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-[0.2em] ${
                        order.paymentStatus === 'paid' ? 'bg-emerald-500 text-emerald-950 border-emerald-400' : 'bg-orange-500/10 text-orange-500 border-orange-500/20'
                    }`}>
                        {order.paymentStatus}
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-10 py-10 border-y border-white/5 border-dashed">
                    <div className="text-center md:text-left">
                        <p className="text-[10px] font-black text-slate-500 uppercase mb-2">Departure</p>
                        <h4 className="text-4xl font-black text-white tracking-tighter">{order.trip.from}</h4>
                    </div>
                    ...
                    <div className="text-center md:text-right">
                        <p className="text-[10px] font-black text-slate-500 uppercase mb-2">Arrival</p>
                        <h4 className="text-4xl font-black text-white tracking-tighter">{order.trip.to}</h4>
                    </div>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-3 gap-8 mt-10">
                    <div>
                        <p className="text-[10px] font-black text-slate-500 uppercase mb-1">Travel Date</p>
                        <p className="text-sm font-bold text-slate-200">{new Date(order.trip.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                    </div>
                    <div>
                        <p className="text-[10px] font-black text-slate-500 uppercase mb-1">Booking ID</p>
                        <p className="text-sm font-mono font-bold text-slate-200">#{order._id.slice(-6).toUpperCase()}</p>
                    </div>
                    <div className="col-span-2 md:col-span-1">
                        <p className="text-[10px] font-black text-slate-500 uppercase mb-1">Method</p>
                        <p className="text-sm font-bold text-emerald-500 uppercase">{order.paymentMethod}</p>
                    </div>
                </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-6 space-y-4">
                ...
                <div className="space-y-1">
                    <p className="text-lg font-black text-white tracking-tight">{order.user.name}</p>
                    <p className="text-xs text-slate-400">{order.user.email}</p>
                </div>
                <div className="pt-4 border-t border-white/5 flex items-center gap-2">
                    <Fingerprint size={14} className="text-slate-600" />
                    <span className="text-[10px] font-mono text-slate-500 truncate">{order.user._id}</span>
                </div>
            </div>
            ...
            <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-6 space-y-4">
                ...
                <div className="space-y-3">
                    <div>
                        <p className="text-[9px] font-bold text-slate-600 uppercase">PIDX</p>
                        <p className="text-[10px] font-mono text-slate-300 truncate">{order.pidx || 'N/A'}</p>
                    </div>
                    <div>
                        <p className="text-[9px] font-bold text-slate-600 uppercase">Transaction ID</p>
                        <p className="text-[10px] font-mono text-emerald-500 truncate">{order.transactionId || 'N/A'}</p>
                    </div>
                </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-8">
            <div className="flex items-center gap-2 mb-6">
                ...
                <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Selected Seats</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
                {order.seats.map((seat: string) => (
                    <div key={seat} className="bg-emerald-500 flex flex-col items-center justify-center p-4 rounded-2xl shadow-lg shadow-emerald-500/10">
                        <span className="text-[10px] font-black uppercase text-emerald-950/50">Seat</span>
                        <span className="text-2xl font-black text-emerald-950">{seat}</span>
                    </div>
                ))}
            </div>
          </div>
